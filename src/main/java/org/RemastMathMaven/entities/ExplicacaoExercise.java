...
public class ExplicacaoExercise {
    ...
    public void setExplicacaoReacoes(List<Forcas> cargasExternas, List<Apoios> apoios) {
        ...
        explicacaoExercise.append("<h1> Cálculo das Reações </h1> Para encontrarmos as reações nos apoios, é necessário verificar " +
                ...
                "não irá girar. O diagrama de corpo livre da viga é: " +
                "<canvas id='dcl-viga' style='100%'></canvas> Portanto, fazendo o " +
                "equilíbrio de forças na vertical, encontra-se: $$ \\sum F_y = 0 \\rightarrow ");
        ...
        boolean omiteMais = true;
            ...
            if (numForcaDistribuida > 0) {
                ...
                explicacaoExercise.append("Em que $x_i$ e $x_f$ representam a posição inicial e final de aplicação da carga, respectivamente, e $w_i$ e $w_f$, os valores, em N/m, iniciais e finais da carga distribuida");
            }
        ...
        for (Forcas cargasExterna : cargasExternas) {
            ...
            for (int i = 0; i < forcasDeReacao.size(); i++) {
                if (i != 0) {
                    if (omiteMais) {
                        explicacaoExercise.append("R_{").append(i + 1).append("}(x_{\\text{").append("apoio").append(" } ").append(i + 1).append("} - x_{\\text{").append("apoio").append(" 1}})");
                        omiteMais = false;
                    } else {
                        explicacaoExercise.append("+R_{").append(i + 1).append("}(x_{\\text{").append("apoio").append(" } ").append(i + 1).append("} - x_{\\text{").append("apoio").append(" 1}})");
                    }
    ...
    public void setResultadoMomentoFletor(List<Forcas> forcas, List<Fletor> listOfFletor) {
        ...
        StringBuilder explicacaoExercise = new StringBuilder();
        explicacaoExercise.append("<h1> Cálculo do Momento Fletor </h1>");
